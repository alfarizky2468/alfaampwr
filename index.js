...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Alfa Admin\n' 
            + 'ORG: Pengembang XBot;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=6285726125902:+62 857-2612-5902\n' 
            + 'END:VCARD' 
prefix = '!'
blocked = []          
/*********** LOAD FILE ***********/
           ...
           *     await client.groupRemove(groupId, sender)
			...
			if (getLevelingLevel(sender) < ahd ) return reply(ind.reglevelahd(command, pushname, getLevelingLevel, sender, ahd))
				...
				if (getLevelingLevel(sender) < ahb ) return reply(ind.reglevelahb(command, pushname, getLevelingLevel, sender, ahb))
					...
					if (args.length < 2) return client.sendMessage(from, 'Mana teks yang ma di jadiin suara? suara aku kah :D?', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 300
				...
				if (getLevelingLevel(sender) < ahb ) return reply(ind.reglevelahb(command, pushname, getLevelingLevel, sender, ahb))
					try {
						if (args.length < 1) return client.sendMessage(from, 'username nama ?', text, {quoted: mek})
						...
						reply('[ERROE] kemungkinan username tidak falid (´-﹏-`；)')
					}
					break
                 case 'linkgc':
				    if (!isGroup) return reply(ind.groupo())
				...
				case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `❮ PESA?? B?OADCAST ❯\n\n${body.slice(4)}`})
						}
			   	...
			   	case 'setpp': 
                       ...
                       media = await client.downloadAndSaveMediaMessage(mek)
                        ...
                        reply('Sukses mengganti ikon grup')
                break						
				case 'add':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					...
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (args[0] === 'buka') {
					    reply(`Berhasil?? Membuka Grup Tod`)
						client.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'tutup') {
						reply(`Berhasil Menutup Grup Tod`)
           ...
           case 'demote':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `yahh jabatan admin kamu sudah dicopot🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`𝘆𝗮𝗵𝗵 @${mentioned[0].split('@')[0]} jabatan admin kamu sudah dicopot🏃`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Asek dapat makanan,otw kick 🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
				...
				if (getLevelingLevel(sender) < ahb ) return reply(ind.reglevelahb(command, pushname, getLevelingLevel, sender, ahb))
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'nihh hyung :v'})
						fs.unlinkSync(ran)
					})
					break
                 case 'simi':
				...
				case 'nsfw':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ SUKSES ❭ Mengaktifkan fitur nswf di group ini')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./src/nsfw.json', JSON.stringify(nsfw))
						reply('❬ SUKSES ❭ Menonaktifkan fitur di group ini')
					} else {
						reply(' *Ketik perintah 1 untuk mengaktifkan, 0 untuk menonaktifkan* \n𝗰𝗼𝗻𝘁𝗼𝗵: 𝗻𝘀𝗳𝘄 𝟭')
					}
					break
            ...
            break
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ SUKSES ❭ Mengaktifkan fitue welcome/left di group ini')
					} else if (Number(args[0]) === 0) {
